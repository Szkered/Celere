...
var exec = argv.exec
if (exec === 'deploy') {
    celere.deployContract(web3, fs, './contract/rtgs.sol', 'Stash', ['JPM', 0],
    			[pubKeys[1], pubKeys[2], pubKeys[4]])
    	.then(() => {
    	    return celere.deployContract(web3, fs, './contract/rtgs.sol', 'Stash', ['DBS', 0],
    				       [pubKeys[1], pubKeys[2], pubKeys[4]]);
    	}).then(() => {
    	    return celere.deployContract(web3, fs, './contract/rtgs.sol', 'Stash', ['UOB', 0],
    				       [pubKeys[1], pubKeys[2], pubKeys[4]]);
    	}).then(() => {
    	    return celere.deployContract(web3, fs, './contract/rtgs.sol', 'PaymentAgent', [],
				       pubKeys.slice(1));
    	});
} else if (exec === 'register') {
    // var Stash = celere.getContract(web3, fs, 'Stash');
    // console.log(Stash.getBalance({from:web3.eth.accounts[0],privateFor:[]}));
    var ta = celere.getContract(web3, fs, 'PaymentAgent');

    console.log(ta.registerStash('JPM', '0x1932c48b2bf8102ba33b4a6b545c32236e342f34',
    				 {from:web3.eth.accounts[0],privateFor:pubKeys.slice(1)}));
    console.log(ta.registerStash('DBS', '0x1349f3e1b8d71effb47b840594ff27da7e603d17',
    				 {from:web3.eth.accounts[0],privateFor:pubKeys.slice(1)}));
    console.log(ta.registerStash('UOB', '0x9d13c6d3afe1721beef56b55d303b09e021e27ab',
    				 {from:web3.eth.accounts[0],privateFor:pubKeys.slice(1)}));
    
} else if (exec === 'pledge') {
    var ta = celere.getContract(web3, fs, 'PaymentAgent');

    console.log(ta.pledge('JPM', 10000, {from:web3.eth.accounts[0],
    					 privateFor:[pubKeys[1]]}));
    console.log(ta.pledge('DBS', 10000, {from:web3.eth.accounts[0],
    					 privateFor:[pubKeys[2]]}));
    console.log(ta.pledge('UOB', 10000, {from:web3.eth.accounts[0],
